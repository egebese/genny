...
export const modelDefinition = z.object({
  ...
  /**
   * The picker card's backdrop, generated by `pnpm cards --art` and served from
   * our own public directory. fal's own thumbnails are screenshots at a dozen
   * sizes, and a picker where every row looks like it came from a different
   * product is one you read instead of scan.
   */
  artUrl: z
    .string()
    .regex(/^\/models\/art\/[\w-]+\.webp$/, 'expected a path like /models/art/flux-dev.webp')
    .optional(),
  /** The provider mark alone, square, drawn on the card and beside the name. */
  markUrl: z
    .string()
    .regex(/^\/models\/marks\/[\w-]+\.svg$/, 'expected a path like /models/marks/flux.svg')
    .optional(),
